...
interface FeedDocs {
    @Operation(summary = "인기 카드 피드 API", description = """
        ...
        - 페이징을 지원하지 않습니다.
    """)
    fun getPopularFeed(latitude: Double?, longitude: Double?, userId: Long): ResponseEntity<List<FeedCardResponse>>
    ...
    """)
    fun getLatestFeed(latitude: Double?, longitude: Double?, lastId: Long?, userId: Long): ResponseEntity<List<FeedCardResponse>>
    ...
    @Operation(summary = "거리순 카드 피드 API", description = """
        ...
        - last 파라미터를 통해 페이징을 지원합니다. (last 파라미터는 이전 요청에서 반환된 카드 피드 리스트의 마지막 카드 ID입니다.)
    """)
    fun getDistanceFeed(latitude: Double, longitude: Double, distance: Double, lastId: Long?, userId: Long): ResponseEntity<List<FeedCardResponse>>
}